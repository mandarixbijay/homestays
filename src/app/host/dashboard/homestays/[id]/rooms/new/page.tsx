...
import type { BedType, Currency, AreaUnit } from "@/types/host-dashboard";
...
export default function NewRoomPage() {
  ...
  const loadMasterData = async () => {
    try {
      setLoading(true);
      const [bedTypesData, currenciesData, areaUnitsData] = await Promise.all([
      ...
      if (currenciesData.length > 0) {
        setFormData((prev) => ({ ...prev, currencyId: currenciesData[0].id.toString() }));
      }
      if (areaUnitsData.length > 0) {
        setFormData((prev) => ({ ...prev, areaUnitId: areaUnitsData[0].id.toString() }));
      }
      if (bedTypesData.length > 0) {
        setFormData((prev) => ({ ...prev, bedTypeId: bedTypesData[0].id.toString() }));
      }
    } catch (error: any) {
      toast({
        title: "Error",
